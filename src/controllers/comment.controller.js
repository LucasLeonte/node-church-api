...
async function list(req, res, next) {
    try {
        ...
        const resource_id = req.query.resource_id
            ? parseInt(req.query.resource_id, 10)
            : undefined;
        const data = await service.list({ page, limit, resource_id });
        res.json(data);
    } catch (err) {
        next(err);
    }
...
async function get(req, res, next) {
    try {
        ...
        if (!row) return next(HttpError.notFound("Comment not found"));
        res.json(row);
    } catch (err) {
        next(err);
    }
...
async function update(req, res, next) {
    try {
        ...
        if (!existing) return next(HttpError.notFound("Comment not found"));
        if (
            !req.user ||
